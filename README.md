...
トップページは、商品販売ではなく健康情報を探しやすくするために、ECサイトの情報導線を参考にした構成です。

- 大きな検索ボックスから症状、筋肉、ストレッチ、SNSで見た健康情報を検索
- 体のセルフチェック、健康情報検索、健康情報ライブラリの3機能へカードで誘導
- 人気検索、カテゴリ、部位、人気記事、新着記事から探せる導線を追加
- SNS健康情報はURL貼り付けだけでなく、記事データベース検索を優先
- 関連記事、関連筋肉、関連症状、関連セルフチェックへ移動しやすい構造
- WebSite、SearchAction、Organizationなどの構造化データでAI検索にも理解されやすい情報設計

トップページのUI本体は `lower-back-assessment/ec-home-ui.js`、専用スタイルは `lower-back-assessment/ec-home.css` に分離しています。
既存のセルフチェック、健康情報ライブラリ、記事自動生成、100テーマ管理、匿名統計は維持しています。
