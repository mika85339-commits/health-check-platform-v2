...
- トップとセルフチェックを白・淡い緑・濃い緑の共通デザインへ統一。
- トップの人体セレクターから、選択部位を保持して既存セルフチェックへ直接進む導線を公開。
- ローカル確認時のGA4・診断記録Functionへの送信防止と、その回帰テストを追加。本番ビルドでは既存の計測・保存処理を維持。
- 肩こり記事: `/health-library/肩こりの原因と日常生活の工夫-鍼灸の可能性/`
- 腰痛記事: `/health-library/腸腰筋の痛みの背景と日常で見直せること-鍼灸という選択肢/`
- 2記事はslug、初回公開日、既存画像、著者を維持し、読者の疑問への回答、確認済み出典、部位別セルフチェック案内、関連記事を更新。

### 2026-09-24

- Sanityの`relatedPosts`を指定順で優先し、自分自身・重複・取得できない記事・非公開記事を除外したうえで、不足分だけ既存の選定ルールから補完する表示へ統一。
- 記事ごとの`diagnosisGuide`を初期HTMLとブラウザ描画の両方へ接続し、未設定記事は従来の案内へフォールバックする構成を追加。
- 肩こり記事: `/health-library/肩こりを和らげるストレッチと鍼灸の可能性/`
  - 公開タイトルを「肩こりにストレッチは役立つ？首・肩の運動と注意点を整理する」へ更新。
  - 慢性頸部痛に対する運動研究の対象範囲と限界、肩こりと腕の動作時痛を同一視しない注意、肩の動きを確認する案内、確認済み出典、関連記事を反映。
- 慢性腰痛記事: `/health-library/運動と慢性痛の関係-生活習慣の見直しと鍼灸の可能性/`
  - 公開タイトルを「慢性腰痛に運動は役立つ？続け方と注意点を整理する」へ更新。
  - 慢性非特異的腰痛に対する運動療法の位置づけと効果の限界、無理なく続けるための注意、腰の動きを確認する案内、確認済み出典、関連記事を反映。
- 2記事とも既存slug、初回公開日、画像、著者を維持する。
